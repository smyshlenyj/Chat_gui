...
int UI(int, char**)
{
	...
	auto connectionID = defaultConnection.connectClientOpen();
	...
	while (!glfwWindowShouldClose(window) && programAlive)
	{
		...
		if (showSignInWindow)
		{
			...
			if (ImGui::Button("Sign in"))
			{
				if (strcmp(login, "_all") == 0 ||
					strcmp(login, "") == 0 ||
					strcmp(password, "") == 0 ||
					!usersDB.loginAndPasswordMatch(connectionID, login, password))
				{
					errorSignIn = true;
				...
				else
				{
					user.setLogin(login);
					user.setPassword(password);
					user.setUserName(usersDB.findUserNameByLogin(connectionID, login));
					loggedIn = true;
					showSignInWindow = false;
					showUsersWindow = true;
		...
		if (showUsersWindow)
		{
			...
			{
				usersDB.refresh(connectionID, user.getLogin());
			...
			int i = 0;
			for (auto element : usersDB.listOfUsers(connectionID, user.getLogin()))
			{
		...
		if (showMessageWindow)
		{
			...
			{
				currentChat = Chat(connectionID, user.getLogin(), selectedRecepient.getLogin());
